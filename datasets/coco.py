...
class COCODataset(Dataset):
    ...
    def pull_image(self, index):
        ...
        img_file = os.path.join(
            self.data_dir, self.image_set, "{:012}".format(id_) + ".jpg"
        )
        ...
        if self.json_file == "instances_val5k.json" and img is None:
            ...
            img_file = os.path.join(
                self.data_dir, "test2017", "{:012}".format(id_) + ".jpg"
            )
            img = cv2.imread(img_file)
...
if __name__ == "__main__":
    ...
    img_size = 640
    dataset = COCODataset(data_dir=data_root, img_size=img_size, transform=transform)
